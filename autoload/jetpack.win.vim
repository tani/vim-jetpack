...
function! s:copy(from, to) abort
  echomsg 'copy from ' . a:from . ' to ' . a:to
...
function! jetpack#bundle() abort
  ...
  for i in range(len(bundle))
    ...
    let srcdir = expand(pkg.path . '/' . pkg.subdir)
    let srcfiles = filter(s:files(srcdir), '!s:ignorable(substitute(v:val, escape(srcdir, "\\"), "", ""))')
    let destfiles = map(copy(srcfiles), 'substitute(v:val, escape(srcdir, "\\"), destdir, "")')
...
function! jetpack#postupdate() abort
  ...
  for pkg in s:pkgs
    ...
    elseif isdirectory(s:optdir . '/' . pkg.name)
      execute printf('cd %s', s:optdir . '/' . pkg.name)
    else
      execute printf('cd %s', s:optdir . '/_')
    endif
